...
function SquadTab({ activeOp, operators, squads, logs, campaigns, onCreate, onJoin, onLeave, onPromote, onDemote, onRemoveMember, onRename, onDisband, raidTemplates, raidInstances, onLaunchRaid }) {
        ...
        return (
          <div>
            ...
            {raidTemplates.length > 0 && (
              <div className="panel">
                <div className="bracket-label">Fastest Clears</div>
                {raidTemplates.map(t => {
                  const completions = raidInstances.filter(r => r.raidTemplateId === t.id && r.status === 'completed' && r.completedAt);
                  const bestPerSquad = {};
