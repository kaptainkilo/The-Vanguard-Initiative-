...
function AdminPanel({ operators, campaigns, logs, onUpdateOperators, onUpdateCampaigns, exercises, protocolSessions, onSaveExercise, onDeleteExercise, onSaveProtocolSession, onDeleteProtocolSession, quips, onSaveQuip, onDeleteQuip, raidTemplates, onSaveRaidTemplate, onDeleteRaidTemplate, campaignPOIs, onSavePOI, onDeletePOI, challengePool, onSaveChallenge, onDeleteChallenge, seasons, onSaveSeason, onDeleteSeason, announcements, onSaveAnnouncement, onDeleteAnnouncement, onGrantAward, onUpdateBirthdateAdmin, quadrants, sectors, systems, planets, viewerIsAdmin, viewerIsModerator, chat, onDeleteChatMessage, onGrantCredits, onGenerateCode }) {
  ...
  const [subTab, setSubTab] = useState(viewerIsAdmin ? 'campaigns' : 'moderation');
  ...
  return (
    <div>
      <div className="radio-group" style={{marginBottom:16}}>
        {viewerIsAdmin && <div className={"radio-opt"+(subTab==='campaigns'?' sel':'')} onClick={()=>setSubTab('campaigns')}>Campaigns</div>}
        {viewerIsAdmin && <div className={"radio-opt"+(subTab==='library'?' sel':'')} onClick={()=>setSubTab('library')}>Content Library</div>}
        {viewerIsAdmin && <div className={"radio-opt"+(subTab==='competitive'?' sel':'')} onClick={()=>setSubTab('competitive')}>Competitive & Events</div>}
        {viewerIsAdmin && <div className={"radio-opt"+(subTab==='roster'?' sel':'')} onClick={()=>setSubTab('roster')}>Roster</div>}
        <div className={"radio-opt"+(subTab==='moderation'?' sel':'')} onClick={()=>setSubTab('moderation')}>Moderation</div>
      </div>
      {subTab==='campaigns' && viewerIsAdmin && (<>
      <div className="panel">
      ...
      {subTab==='competitive' && viewerIsAdmin && (<>
      ...
      <div className="panel">
        ...
        <table>
          <thead><tr><th>Callsign</th><th>Weekly Target</th><th>Age Division</th><th>Birthdate</th><th>Admin</th><th>Mod</th><th>Command Rank</th><th>Credits</th><th></th><th></th></tr></thead>
          <tbody>
            {operators.map(o => (
              <tr key={o.id}>
                ...
                <td className="mono" style={{fontSize:11}}>{o.requisitionCredits||0}</td>
      ...
      <div className="panel">
        <div className="bracket-label">Grant Requisition Credits</div>
        <div className="info-note" style={{marginBottom:12}}>For Ko-fi supporters — Command's thanks, not a purchase. No automatic link between a donation and this grant; when someone lets you know they've supported the Initiative, credit them here. Rough guideline: ~50 credits per $1, adjust by feel.</div>
        <div className="field"><label>Operator</label>
          <select value={creditsFor||''} onChange={e=>setCreditsFor(e.target.value||null)}>
            <option value="">Select an Operator...</option>
            {operators.map(o => <option key={o.id} value={o.id}>{o.callsign} ({o.requisitionCredits||0} credits)</option>)}
          </select>
        </div>
        {creditsFor && (
          <div style={{border:'1px solid var(--amber)',borderRadius:2,padding:12,marginTop:14}}>
      ...
      <div className="panel">
        <div className="bracket-label">Generate Redemption Code</div>
        <div className="info-note" style={{marginBottom:12}}>Single-use, flat value. Generate one, copy it, send it to the supporter yourself (Ko-fi reply, DM, however you're already in touch). They redeem it themselves from their own Dossier.</div>
        <div style={{display:'flex',gap:8,alignItems:'flex-end',marginBottom:14}}>
          <div className="field" style={{marginBottom:0}}><label>Credits</label><input type="number" value={codeAmount} onChange={e=>setCodeAmount(e.target.value)} placeholder="e.g. 250" style={{width:100}} /></div>
          <button className="primary small" disabled={codeBusy} onClick={async ()=>{
            if (!codeAmount || Number(codeAmount)<=0) return;
            setCodeBusy(true);
        ...
        {generatedCodes.length > 0 && (
          <div>
            ...
            {generatedCodes.map((g,i) => (
              <div key={i} className="field-row"><span className="mono amber">{g.code}</span><span className="dim mono" style={{fontSize:11}}>{g.credits} credits</span></div>
            ))}
          </div>
        )}
      </div>
        ...
        {subTab==='moderation' && (<>
      <div className="panel">
        <div className="bracket-label">Grant Award</div>
        <div className="field"><label>Operator</label>
          <select value={grantAwardFor||''} onChange={e=>{setGrantAwardFor(e.target.value||null); setGrantAwardDraft({title:'', description:''});}}>
            <option value="">Select an Operator...</option>
            {operators.map(o => <option key={o.id} value={o.id}>{o.callsign}</option>)}
          </select>
        </div>
        {grantAwardFor && (
          <div style={{border:'1px solid var(--amber)',borderRadius:2,padding:12,marginTop:14}}>
      ...
      <div className="panel">
        <div className="bracket-label">Chat Moderation {'\u2014'} Main</div>
        <div className="info-note" style={{marginBottom:12}}>Deletions apply immediately for you. Other operators will see the message clear the next time their app refreshes data {'\u2014'} chat's live-update feed only watches for new messages, not removals.</div>
        <div style={{maxHeight:400,overflowY:'auto'}}>
          {(chat||[]).filter(m=>m.channel==='main').slice().sort((a,b)=>b.timestamp-a.timestamp).slice(0,50).map(m => (
            <div key={m.id} className="field-row" style={{alignItems:'flex-start'}}>
              <div style={{flex:1}}>
                <span style={{fontSize:11,fontWeight:600,color: m.isCommand?'var(--amber)':'var(--text)'}}>{m.authorName}</span>
                <span className="dim mono" style={{fontSize:10,marginLeft:6}}>{new Date(m.timestamp).toLocaleString()}</span>
                <div style={{fontSize:12,marginTop:2}}>{m.text}</div>
              </div>
              <button className="ghost small" onClick={()=>onDeleteChatMessage(m.id)}>Delete</button>
            </div>
          ))}
          {(chat||[]).filter(m=>m.channel==='main').length === 0 && <div className="dim" style={{fontSize:12}}>No messages in Main yet.</div>}
        </div>
      </div>
        </>)}
    </div>
