...
const NAV_STRUCTURE = [
  ...
  { type:'item', key:'tcs', label:'Tactical Command Sim' },
...
// Solo leaderboards — every operator gets a personal ranking regardless of
// Squad membership, matching the established "Squads never penalize solo
// operators" principle. Reuses computeMCP/computeORS directly rather than
// re-deriving the numbers, so these can never drift from what an operator's
// own Dossier already shows them.
function computeIndividualMCPRankings(operators, campaigns, logs, season) {
  const seasonLogs = season ? logs.filter(l => l.date >= season.startDate && l.date <= season.endDate) : logs;
  return operators.filter(o=>o.onboarded).map(o => ({ operator: o, mcp: computeMCP(o.id, campaigns, seasonLogs) })).sort((a,b)=>b.mcp-a.mcp);
}
function computeIndividualORSRankings(operators, logs) {
  return operators.filter(o=>o.onboarded).map(o => ({ operator: o, ors: computeORS(o.id, o, logs).ors })).sort((a,b)=>b.ors-a.ors);
}
...
// Faction icons live in the repo's Images/Enemyicons folder. Referenced
// relatively, matching how js/*.js is already referenced from index.html
// (no repo-name prefix in the path) — if a path turns out to be wrong,
// onError below hides the icon quietly instead of showing a broken image.
...
function EnemyIcon({ faction, size }) {
  const s = size || 44;
  const src = ENEMY_ICON_PATHS[(faction||'').toLowerCase()];
...
// ============================================================
// TACTICAL COMMAND SIMULATION — MATCH ENGINE (Stage 5a)
// Pure functions only — no DB calls here, matching this file's existing
// role as the computation layer. Persistence (creating/updating a match
// row) lives in app-core.js, same split as everywhere else in this app.
//
// AI SCOPING NOTE: the AI does not draw/play cards from a hand or manage
// Energy — it starts with its Hero plus a fixed set of Units already
// deployed, and only ever moves/attacks with what it started with. This
// is a deliberate simplification of "simple heuristic AI" from the
// original design discussion (which described attack/move/Cover-seeking
// behavior, not card-playing behavior) — flagging it clearly since it
// wasn't explicitly re-confirmed after that round.
//
// Combat math and terrain are the parts most worth trusting; turn-flow
// edge cases (exact bleed/stun timing across a full turn cycle) are a
// reasonable first pass, not something that's been played and tuned yet.
// ============================================================

const TCS_BOARD_COLS = 5;
const TCS_BOARD_ROWS = 8;
const TCS_COVER_TILES = [[1,3],[3,3],[1,4],[3,4]];
const TCS_HAZARD_TILES = [[2,2],[2,5]];

function tcsGenerateBoard() {
  const tiles = {};
  for (let c=0;c<TCS_BOARD_COLS;c++) for (let r=0;r<TCS_BOARD_ROWS;r++) {
    let terrain = 'open';
    if (TCS_COVER_TILES.some(([cc,rr])=>cc===c&&rr===r)) terrain = 'cover';
    if (TCS_HAZARD_TILES.some(([cc,rr])=>cc===c&&rr===r)) terrain = 'hazard';
    tiles[c+','+r] = { terrain };
  }
  return tiles;
}

function tcsShuffle(arr) {
  const a = arr.slice();
  for (let i=a.length-1;i>0;i--) { const j=Math.floor(Math.random()*(i+1)); const t=a[i]; a[i]=a[j]; a[j]=t; }
  return a;
}

function tcsInitMatch(deck, allCards, aiRoster) {
  const heroCard = allCards.find(c=>c.id===deck.heroCardId);
  const shuffled = tcsShuffle(deck.cardIds);
  const hand = shuffled.splice(0,5);

  const units = [];
  units.push({ instanceId:'p_hero', cardId: heroCard.id, owner:'player', col:2, row:7, currentHp: heroCard.hp, exhausted:false, stunned:false, bleeding:0 });

  const aiHero = aiRoster.find(c=>c.cardType==='hero');
  units.push({ instanceId:'ai_hero', cardId: aiHero.id, owner:'ai', col:2, row:0, currentHp: aiHero.hp, exhausted:false, stunned:false, bleeding:0 });
  aiRoster.filter(c=>c.cardType==='unit').slice(0,2).forEach((c,i) => {
    units.push({ instanceId:'ai_u'+i, cardId: c.id, owner:'ai', col: i===0?1:3, row:1, currentHp: c.hp, exhausted:false, stunned:false, bleeding:0 });
  });

  return {
    tiles: tcsGenerateBoard(),
    units: units,
    playerHand: hand,
    playerDraw: shuffled,
    playerDiscard: [],
...
function tcsUnitCard(state, allCards, instanceId) {
  const unit = state.units.find(u=>u.instanceId===instanceId);
  if (!unit) return null;
  return allCards.find(c=>c.id===unit.cardId) || null;
}

// Deploys a Unit or Deployable from hand onto the board. Gear and
// Maneuver mechanical effects (equip bonuses, one-shot effects) aren't
// built yet — those need their own engine work (applying persistent
// modifiers to combat resolution) beyond what Stage 5a covers. Flagging
// clearly rather than letting it silently not work if someone plays one.
//
// Newly deployed units start exhausted (can't act the same turn they're
// played) — a standard TCG "summoning sickness" convention, not something
// explicitly agreed in the original design discussion. Worth knowing this
// was assumed, not confirmed.
function tcsPlayCard(state, allCards, cardId, targetCol, targetRow) {
  const card = allCards.find(c=>c.id===cardId);
  if (!card) return { success:false, message:'Unknown card.' };
  if (card.cardType !== 'unit' && card.cardType !== 'deployable') return { success:false, message:'Gear and Maneuvers aren\'t playable yet — Units and Deployables only for now.' };
  const handIdx = state.playerHand.indexOf(cardId);
  if (handIdx === -1) return { success:false, message:'Card not in hand.' };
  if (state.playerEnergy < (card.cost||0)) return { success:false, message:'Not enough Energy.' };
  if (targetRow < 6 || targetRow > 7) return { success:false, message:'Must deploy within your deployment zone (the bottom two rows).' };
  const occupied = state.units.some(u=>u.currentHp>0 && u.col===targetCol && u.row===targetRow);
  if (occupied) return { success:false, message:'Tile occupied.' };

  state.units.push({ instanceId: 'p_'+cardId+'_'+Date.now(), cardId: cardId, owner:'player', col: targetCol, row: targetRow, currentHp: card.hp, exhausted: true, stunned:false, bleeding:0 });
  state.playerHand.splice(handIdx,1);
  state.playerEnergy -= (card.cost||0);
  return { success:true, state: state };
}

function tcsMoveUnit(state, allCards, instanceId, targetCol, targetRow) {
  const unit = state.units.find(u=>u.instanceId===instanceId);
  if (!unit || unit.currentHp<=0) return { success:false, message:'Invalid unit.' };
  if (unit.owner==='player' && state.playerEnergy < 1) return { success:false, message:'Not enough Energy.' };
  if (unit.stunned) return { success:false, message:'Stunned — cannot act.' };
  const card = allCards.find(c=>c.id===unit.cardId);
  if (targetCol<0||targetCol>=TCS_BOARD_COLS||targetRow<0||targetRow>=TCS_BOARD_ROWS) return { success:false, message:'Off the board.' };
  const dist = Math.abs(unit.col-targetCol)+Math.abs(unit.row-targetRow);
  if (dist === 0 || dist > (card.movement||0)) return { success:false, message:'Out of movement range.' };
  const occupied = state.units.some(u=>u.currentHp>0 && u.col===targetCol && u.row===targetRow);
  if (occupied) return { success:false, message:'Tile occupied.' };
  unit.col = targetCol; unit.row = targetRow;
  if (unit.owner==='player') state.playerEnergy -= 1;
  return { success:true, state: state };
}

function tcsAttack(state, allCards, attackerInstanceId, targetInstanceId) {
  const attacker = state.units.find(u=>u.instanceId===attackerInstanceId);
  const target = state.units.find(u=>u.instanceId===targetInstanceId);
  if (!attacker || !target || attacker.currentHp<=0 || target.currentHp<=0) return { success:false, message:'Invalid target.' };
  if (attacker.exhausted) return { success:false, message:'Already acted this turn.' };
  if (attacker.stunned) return { success:false, message:'Stunned — cannot act.' };
  const card = allCards.find(c=>c.id===attacker.cardId);
  const dist = Math.abs(attacker.col-target.col)+Math.abs(attacker.row-target.row);
  const maxRange = card.rangeType==='melee' ? 1 : (card.rangeDistance||1);
  if (dist > maxRange) return { success:false, message:'Out of range.' };

  const targetCard = allCards.find(c=>c.id===target.cardId);
  const targetTile = state.tiles[target.col+','+target.row];
  const ignoresCover = card.critKeyword === 'pierce';
  const coverBonus = (targetTile.terrain==='cover' && !ignoresCover) ? 2 : 0;
  const defense = (targetCard.defense||0) + coverBonus;

  const roll = 1+Math.floor(Math.random()*8);
  let outcome, damage = 0;
  if (roll === 1) {
    outcome = 'miss';
  } else if (roll === 8) {
    outcome = 'crit';
    damage = (card.damage||0) * (card.critKeyword==='extra_hit' ? 2 : 1);
  } else if ((roll + (card.attackMod||0)) >= defense) {
    outcome = 'hit';
  ...
  if (outcome !== 'miss') {
    ...
    if (outcome==='crit' && card.critKeyword==='bleed') target.bleeding = (target.bleeding||0) + 2;
    if (outcome==='crit' && card.critKeyword==='stun') target.stunned = true;
  }
  attacker.exhausted = true;
  state.log = (state.log||[]).concat([{ turn: state.turnNumber, attackerId: attacker.instanceId, targetId: target.instanceId, roll: roll, outcome: outcome, damage: damage }]);
  return { success:true, state: state, roll: roll, outcome: outcome, damage: damage };
}

function tcsCheckWinCondition(state) {
  const playerHero = state.units.find(u=>u.instanceId==='p_hero');
  const aiHero = state.units.find(u=>u.instanceId==='ai_hero');
  if (playerHero && playerHero.currentHp <= 0) return 'lost';
  if (aiHero && aiHero.currentHp <= 0) return 'won';
  if (state.turnNumber > 8) {
    if (!playerHero || !aiHero) return 'draw';
    if (playerHero.currentHp === aiHero.currentHp) return 'draw';
    return playerHero.currentHp > aiHero.currentHp ? 'won' : 'lost';
  }
  return null; // still active
}

// Simple heuristic AI: attack the best-odds target in range if one
// exists, otherwise move toward the nearest player unit, preferring
// Cover tiles when a move choice is otherwise close.
function tcsAiTakeTurn(state, allCards) {
  const aiUnits = state.units.filter(u=>u.owner==='ai' && u.currentHp>0);
  aiUnits.forEach(u => { u.stunned = false; }); // stun only lasts until the stunned unit's own next turn start
  aiUnits.forEach(unit => {
    if (unit.stunned) return;
    const card = allCards.find(c=>c.id===unit.cardId);
    const maxRange = card.rangeType==='melee' ? 1 : (card.rangeDistance||1);
    const playerUnits = state.units.filter(u=>u.owner==='player' && u.currentHp>0);
    if (playerUnits.length === 0) return;

    const inRange = playerUnits.filter(t => Math.abs(unit.col-t.col)+Math.abs(unit.row-t.row) <= maxRange);
    if (inRange.length > 0) {
      // best odds = lowest effective defense target
      inRange.sort((a,b) => {
        const ta = allCards.find(c=>c.id===a.cardId), tb = allCards.find(c=>c.id===b.cardId);
        return (ta.defense||0) - (tb.defense||0);
      });
      unit.exhausted = false; // AI units aren't limited by the player's energy system
      tcsAttack(state, allCards, unit.instanceId, inRange[0].instanceId);
      return;
    }

    // No target in range — move toward the nearest player unit, mildly
    // preferring a Cover tile if one is a reasonable step toward them
    let nearest = playerUnits[0], nearestDist = 999;
    playerUnits.forEach(t => { const d = Math.abs(unit.col-t.col)+Math.abs(unit.row-t.row); if (d<nearestDist) { nearestDist=d; nearest=t; } });
    const moveRange = card.movement||1;
    let bestTile = null, bestScore = -Infinity;
    for (let dc=-moveRange;dc<=moveRange;dc++) for (let dr=-moveRange;dr<=moveRange;dr++) {
      const nc = unit.col+dc, nr = unit.row+dr;
      if (Math.abs(dc)+Math.abs(dr) > moveRange || Math.abs(dc)+Math.abs(dr)===0) continue;
      if (nc<0||nc>=TCS_BOARD_COLS||nr<0||nr>=TCS_BOARD_ROWS) continue;
      if (state.units.some(u=>u.currentHp>0 && u.col===nc && u.row===nr)) continue;
      const distAfter = Math.abs(nc-nearest.col)+Math.abs(nr-nearest.row);
      let score = -distAfter; // closer is better
      if (state.tiles[nc+','+nr].terrain==='cover') score += 0.5;
      if (state.tiles[nc+','+nr].terrain==='hazard') score -= 2;
      if (score > bestScore) { bestScore = score; bestTile = {col:nc,row:nr}; }
    }
    if (bestTile) { unit.col = bestTile.col; unit.row = bestTile.row; }
  });
  return state;
}

function tcsEndPlayerTurn(state, allCards) {
  // End-of-turn effects for player units, then hand back to the AI
  state.units.filter(u=>u.owner==='player' && u.currentHp>0).forEach(u => {
    const tile = state.tiles[u.col+','+u.row];
    if (tile.terrain==='hazard') u.currentHp -= 1;
    if (u.bleeding>0) { u.currentHp -= 1; u.bleeding -= 1; }
  });
  let result = tcsCheckWinCondition(state);
  if (result) return { state, result };

  tcsAiTakeTurn(state, allCards);
  state.units.filter(u=>u.owner==='ai' && u.currentHp>0).forEach(u => {
    const tile = state.tiles[u.col+','+u.row];
    if (tile.terrain==='hazard') u.currentHp -= 1;
    if (u.bleeding>0) { u.currentHp -= 1; u.bleeding -= 1; }
  });
  result = tcsCheckWinCondition(state);
  if (result) return { state, result };

  // Back to player: draw, refresh energy, reset exhaustion/stun
  state.turnNumber += 1;
  state.playerEnergy = 3;
  const drawCount = Math.max(0, 5 - state.playerHand.length);
  if (drawCount > 0) {
    const drawn = state.playerDraw.splice(0, drawCount);
    state.playerHand = state.playerHand.concat(drawn);
  }
  state.units.filter(u=>u.owner==='player').forEach(u => { u.exhausted = false; u.stunned = false; });
