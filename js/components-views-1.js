  ...
  if (!mySquad) {
    return (
      <div>
        ...
        <div className="grid2">
          <div className="panel">
            <div className="bracket-label">Leaderboard — Individual MCP{currentSeason ? ' ('+currentSeason.name+')' : ' (All-Time)'}</div>
            {computeIndividualMCPRankings(operators, campaigns, logs, currentSeason).slice(0,5).map((r,i) => (
              <div key={r.operator.id} className="stat-row"><span>{i+1}. {r.operator.callsign}</span><span className="stat-val">{r.mcp}</span></div>
            ))}
          </div>
          <div className="panel">
            <div className="bracket-label">Leaderboard — Individual ORS (Current)</div>
            {computeIndividualORSRankings(operators, logs).slice(0,5).map((r,i) => (
              <div key={r.operator.id} className="stat-row"><span>{i+1}. {r.operator.callsign}</span><span className="stat-val">{r.ors}</span></div>
            ))}
          </div>
        </div>
      </div>
...
function Codex({ entries, isAdmin, onUpdate }) {
  ...
  function startEdit(entry) { setEditingId(entry.id); setDraft({title:entry.title, body:entry.body, iconRef:entry.iconRef||'', category:entry.category}); }
  function saveEdit() {
        ...
        <div key={entry.id} className="codex-entry">
          {editingId === entry.id ? (
            <div>
              ...
              <input type="text" value={draft.iconRef} onChange={e=>setDraft(Object.assign({},draft,{iconRef:e.target.value}))} placeholder="Icon refs, e.g. rank:Operator:2,specialty:Recon,award:streak_30,enemy:kharvax" style={{marginBottom:8}} />
...
function personalBest(operatorId, logs, exerciseName) {
  const prior = logs.filter(l => l.type==='protocol' && l.operatorId===operatorId && l.variant===exerciseName);
  if (prior.length === 0) return null;
  return Math.max.apply(null, prior.map(l=>l.totalValue));
}
function lastLoggedExercise(operatorId, logs, exerciseName) {
  const prior = logs.filter(l => l.type==='protocol' && l.operatorId===operatorId && l.variant===exerciseName);
  if (prior.length === 0) return null;
