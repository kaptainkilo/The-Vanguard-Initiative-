...
function rowToTcsCard(c) { return { id: c.id, name: c.name, cardType: c.card_type, category: c.category, isPveOnly: c.is_pve_only, rarity: c.rarity, cost: c.cost, hp: c.hp, defense: c.defense, attackMod: c.attack_mod, damage: c.damage, rangeType: c.range_type, rangeDistance: c.range_distance, movement: c.movement, critKeyword: c.crit_keyword, description: c.description }; }
function rowToTcsCollectionEntry(e) { return { id: e.id, operatorId: e.operator_id, cardId: e.card_id, quantity: e.quantity, acquiredAt: e.acquired_at }; }
...
async function fetchAllData() {
  ...
  const { data: tcsCardRows } = await sb.from('tcs_cards').select('*');
  const tcsCards = (tcsCardRows||[]).map(rowToTcsCard);
  const { data: tcsCollectionRows } = await sb.from('tcs_player_collections').select('*');
  const tcsCollections = (tcsCollectionRows||[]).map(rowToTcsCollectionEntry);

  const { data: tcsMatchRows } = await sb.from('tcs_matches').select('*');
  const tcsMatches = (tcsMatchRows||[]).map(m => ({ id: m.id, operatorId: m.operator_id, deckId: m.deck_id, opponentType: m.opponent_type, opponentFaction: m.opponent_faction, status: m.status, turnNumber: m.turn_number, boardState: m.board_state, updatedAt: m.updated_at }));

  const { data: tcsDeckRows } = await sb.from('tcs_decks').select('*');
  const { data: tcsDeckCardRows } = await sb.from('tcs_deck_cards').select('*');
  const tcsDecks = (tcsDeckRows||[]).map(d => ({
    id: d.id, operatorId: d.operator_id, name: d.name, heroCardId: d.hero_card_id,
    cardIds: (tcsDeckCardRows||[]).filter(dc=>dc.deck_id===d.id).map(dc=>dc.card_id),
  }));
